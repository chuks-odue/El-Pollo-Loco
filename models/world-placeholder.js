...
class World {
    ...
    sounds = {
        ...
        'coin-lost': new Audio(''),
        'lose': new Audio('audio/lost-game sound.wav'),
        ...
        setInterval(() => {
            ...
            this.level.enemies.forEach((enemy) => {
                if (!enemy.isDead && this.character.isColliding(enemy)) {
                    if (this.character.y + this.character.height * 0.8 < enemy.y + enemy.height * 0.5 && !(enemy instanceof SmallChicken)) {
                        // Character is on top of a normal chicken, kill the chicken
                        enemy.die();
                    } else {
                        // Character is not on top of the enemy or it's a small chicken, hurt the character
                        this.character.hit();
...
class World {
    ...
    playSound(name) {
        if (!this.soundEnabled || this.gameOverImageShown) return; 
